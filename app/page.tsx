...
const MAIL = "eczpsknuraycicek@gmail.com";
const appointment = `mailto:${MAIL}?subject=Randevu%20Talebi`;
...
function Header() {
  ...
  return <header className={`site-header ${scrolled ? "scrolled" : ""}`}><div className="container nav-wrap">
    ...
    <nav className={`main-nav ${open ? "open" : ""}`} aria-label="Ana menü">
      {[["#about", "Hakkımda"], ["#education", "Eğitim ve Sertifikalar"], ["#services", "Çalışma Alanları"], ["#process", "Terapi Süreci"], ["#faq", "SSS"], ["#contact", "İletişim"]].map(([href, label]) => <a key={href} href={href} onClick={() => setOpen(false)}>{label}</a>)}
      <a className="nav-appointment" href={appointment}>Randevu Al</a>
    </nav>
    <a className="instagram-link" href="https://www.instagram.com/eczpsknuray/" target="_blank" rel="noreferrer" aria-label="Instagram hesabını yeni sekmede aç">
      <svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="5" /><circle cx="12" cy="12" r="4" /><circle className="instagram-dot" cx="17.5" cy="6.5" r="1" /></svg>
      <span>E-randevu</span>
    </a>
  </div></header>;
}
...
const supportAreas = [
  ["⌁", "Kaygı ve Stres", "Günlük yaşamda kaygı, yoğun stres, endişe ve zihinsel yoğunlukla başa çıkma."],
  ["◒", "Depresyon ve Duygudurum", "İçe kapanma, isteksizlik, çökkünlük ve yaşamdan alınan keyifte azalma üzerine çalışma."],
  ["◐", "Öfke Problemleri", "Öfkeyi fark etme, ifade etme ve daha sağlıklı biçimde yönetme."],
  ["◌", "Özgüven, Özdeğer ve Yetersizlik Hissi", "Kendini yetersiz veya değersiz hissetme, kendini eleştirme ve kişinin kendisiyle daha sağlıklı bir ilişki kurması."],
  ["◎", "İlişkiler ve Bağlanma", "Romantik ilişkilerde yaşanan sorunlar, iletişim güçlükleri, sınırlar ve tekrarlayan ilişki döngüleri."],
  ["⌂", "Aile ve Aile İçi İlişkiler", "Aile içi iletişim, çatışmalar, sınır koyma ve ilişkisel sorunlar üzerine çalışma."],
  ["◑", "Kayıp ve Yas Süreci", "Yaşanan kayıpların ardından ortaya çıkan duygularla baş etme ve yas sürecine eşlik etme."],
  ["◍", "Sosyal Beceriler", "İletişim kurma, kendini ifade etme, sınır koyma ve sosyal ortamlarda daha rahat hissetme."],
];
const training = [["2025", "Aile Danışmanlığı Eğitimi", "MEB Tecrübe Akademi · Psikoterapist Gökhan Çınar"], ["2025", "Türk Psikologlar Derneği Sürekli Eğitim Merkezi – MMPI Uygulama ve Yorumlama", "Dr. Öğr. Üyesi Sezin Andiç · 32 saat"], ["2024", "Şema Terapi", "Dr. Psikolog Ayça Ferda Kansu"], ["2024", "Kabul ve Adanmışlık Terapisi Eğitimi (ACT)", "Doç. Dr. Çağdaş Öykü Meriç · 36 saat"]];
...
export default function Home() {
  ...
  return <><Header /><main id="main">
    <section className="hero" id="hero"><div className="hero-shape" /><div className="container hero-grid"><div className="hero-copy reveal"><p className="eyebrow">Online bireysel terapi</p><h1>Şema Terapi &amp; Kabul ve Kararlılık Terapisi (ACT)</h1><div className="hero-actions"><a className="button button-primary" href={appointment}>Randevu Al <span>→</span></a><a className="button button-text" href="#about">Beni tanıyın <span>↓</span></a></div></div><div className="hero-image-wrap reveal"><div className="image-arch"><Image src="/images/arkaplan1.jpg" alt="Eczacı Klinik Psikolog Nuray Çiçek" fill priority sizes="(max-width: 850px) 90vw, 415px" /></div></div></div></section>
    <section className="intro-strip"><div className="container"><p>“Terapi, iki insanın gerçek karşılaşmasıdır.”</p><span>— Irvin D. Yalom</span></div></section>
  ...
  </main><footer><div className="container footer-inner"><a className="brand" href="#hero">Nuray <span>Çiçek</span><small>Eczacı Klinik Psikolog</small></a><a className="design-credit" href="https://wa.me/905555597505" target="_blank" rel="noreferrer" aria-label="Berkan Satın'a WhatsApp üzerinden mesaj gönder">Tasarım Destek © 2026 v1.2 Berkan Satın</a><div><a href={`mailto:${MAIL}?subject=KVKK%20ve%20Gizlilik%20Metni`}>KVKK &amp; Gizlilik</a><a href={`mailto:${MAIL}?subject=İptal%20Şartları`}>İptal Şartları</a></div></div></footer><a className="whatsapp" href={appointment} aria-label="Randevu için e-posta gönderin">✉<span>Randevu</span></a>
  </>;
}
